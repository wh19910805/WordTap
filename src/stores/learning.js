...
import { updateSource, play, stop as stopAudioPlay, getIsPlaying } from '@/composables/audio'
...
export const useLearningStore = defineStore('learning', () => {
  ...
  function autoFillNames(lessonData) {
    if (!settingsStore.defaultShowName || !lessonData?.nameList || lessonData.nameList.length === 0) {
      return
    }
    ...
    lessonData.sentences.forEach((sentence, sentenceIndex) => {
      ...
      for (const name of nameList) {
        ...
        // 使用大小写不敏感匹配，检查是否在句子开头
        const regex = new RegExp(`^${escapedName}`, 'i')
        const match = sentenceText.match(regex)
        
        if (match) {
          // 找到句首的人名
          const matchedName = match[0] // 使用原文中的实际大小写
          let filledText = matchedName
          let charIndex = matchedName.length
          
          // 检查人名后面是否有标点符号或空格，也自动填充
          while (charIndex < sentenceText.length) {
            const nextChar = sentenceText[charIndex]
            if (isPunctuation(nextChar) || isSpace(nextChar)) {
              filledText += nextChar
              charIndex++
            } else {
              break
  ...
  function handleInput(char) {
    ...
    // 自动跳过标点符号和空格
    while (expectedChar && (isPunctuation(expectedChar) || isSpace(expectedChar))) {
      inputText.value += expectedChar
      currentCharIndex.value++
      // 更新当前行的进度
      if (!sentenceProgress.value[currentSentenceIndex.value]) {
        sentenceProgress.value[currentSentenceIndex.value] = { completed: false, charIndex: 0, inputText: '' }
      }
      sentenceProgress.value[currentSentenceIndex.value].charIndex = currentCharIndex.value
      expectedChar = currentSentence.value.text[currentCharIndex.value]
    }
    ...
    const isFirstLetter = !hasLetterInCurrentLine && isLetter(char) && isLetter(expectedChar)
    ...
    if (isMatch) {
      ...
      // 输入正确后，自动跳过后续的标点和空格，移动到下一个单词
      let nextChar = currentSentence.value.text[currentCharIndex.value]
      while (nextChar && (isPunctuation(nextChar) || isSpace(nextChar))) {
        inputText.value += nextChar
        currentCharIndex.value++
        sentenceProgress.value[currentSentenceIndex.value].charIndex = currentCharIndex.value
        sentenceProgress.value[currentSentenceIndex.value].inputText = inputText.value
        nextChar = currentSentence.value.text[currentCharIndex.value]
      }
  ...
  function isCharHinted(sentenceIndex, charIndex) {
    ...
    return hints.some(hint => charIndex >= hint.wordStart && charIndex < hint.wordEnd)
  }
